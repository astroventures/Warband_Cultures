...
#           (try_end),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  ########################################################
  ##  WARBAND CULTURES MOD - DAILY CULTURE UPDATE  #######
  ########################################################
  # Task 2.3: Daily culture shift trigger
  # Iterates all settlements and applies culture shift toward owning faction
  (24,  # Every 24 hours
   [
      (try_for_range, ":center", centers_begin, centers_end),
  ...
  # Warband Cultures: Weekly maintenance for custom troops (168h = 7 days)
  (168,
   [
      # Calculate total maintenance for all custom troops in player's forces
      (assign, ":total_maintenance", 0),

      # Iterate through all template troops to find configured ones
      (try_for_range, ":troop", "trp_wc_template_001", "trp_wc_templates_end"),
        (troop_slot_eq, ":troop", wc_slot_troop_is_configured, 1),

        # Count in player party
        (party_count_companions_of_type, ":count", "p_main_party", ":troop"),
        (try_begin),
          (gt, ":count", 0),
          (troop_get_slot, ":maint", ":troop", wc_slot_troop_maintenance_cost),
          (store_mul, ":troop_maint", ":maint", ":count"),
          (val_add, ":total_maintenance", ":troop_maint"),
        (try_end),

        # Count in player's garrisons
        (try_for_range, ":center", walled_centers_begin, walled_centers_end),
          (party_slot_eq, ":center", slot_town_lord, "trp_player"),
          (party_count_companions_of_type, ":count", ":center", ":troop"),
          (try_begin),
            (gt, ":count", 0),
            (troop_get_slot, ":maint", ":troop", wc_slot_troop_maintenance_cost),
            (store_mul, ":troop_maint", ":maint", ":count"),
      ...
      # Deduct maintenance from player gold
      (try_begin),
        (gt, ":total_maintenance", 0),
        (store_troop_gold, ":gold", "trp_player"),
        (try_begin),
          (ge, ":gold", ":total_maintenance"),
          (troop_remove_gold, "trp_player", ":total_maintenance"),
        (else_try),
          # Not enough gold - deduct what we can, morale penalty could go here
          (troop_remove_gold, "trp_player", ":gold"),
        (try_end),
      (try_end),

      # Also apply vassal cost sharing if enabled
      (call_script, "script_wc_apply_vassal_cost_sharing"),
   ]),
  (24,
   []),
  (24,
